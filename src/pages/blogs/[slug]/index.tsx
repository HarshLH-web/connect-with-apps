...
function BlogDetailPage({ blog }: BlogDetailPageProps) {
  ...
  useEffect(() => {
    ...
    const processImages = (htmlContent: string): string => {
      const tempDiv = document.createElement("div");
      ...
      images.forEach((img, index) => {
        ...
        if (img.parentNode && placeholder.firstChild) {
  ...
  return (
    <>
      <Head>
        ...
        <meta
          property="og:url"
          content={`https://connectwithapps.com/blogs/${blog.slug}`}
        />
      ...
      <div
        id="blog-container"
        className="w-[95%] lg:w-[90%] max-w-7xl mx-auto pb-48 pt-4 lg:pt-32 lg:pb-60 bg-gray-50 border-x border-gray-200 mb-[-130px]"
      >
        <div
          className="mx-2 lg:mx-16 flex items-center justify-between h-52 lg:h-72 bg-cover bg-center bg-no-repeat rounded-xl"
          style={{
            boxShadow: "inset 0px 0px 400px 54px rgba(0,0,0,0.66)",
            backgroundImage: `url(${
              isMobile ? blog.mobileBannerImage : blog.desktopBannerImage
            })`,
          }}
        >
          <h1
            className="text-3xl font-bold max-w-176 mx-auto text-white text-center px-2"
...
export const getStaticPaths: GetStaticPaths = async () => {
  try {
    const res = await axios.get(
      "https://webpanel.store/api/blogs/selected-fields"
    );
    const blogs: Blog[] = res.data.filter((blog: Blog) => blog.toPublish);
...
export const getStaticProps: GetStaticProps<BlogDetailPageProps> = async ({
  ...
  try {
    const response = await axios.get(
      `https://webpanel.store/api/blogs/${slug}`
    );
    return {
      props: {
        blog: response.data,
